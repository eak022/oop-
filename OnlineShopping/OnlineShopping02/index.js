class Customer {
  ...
  //class customer เก็บชื่อ ที่อยู่ และเก็บข้อมูลการสั่งซื้อของลูกค้าไว้ใน orders
  //addOrder เพิ่มคำสั่งซื้อลงใน orders
  ...
  // Class Order เก็บวันที่ (date) และสถานะ (status) รวมทั้งเก็บข้อมูลการชำระเงินใน payment และรายละเอียดคำสั่งซื้อใน orderDetails
 // calcSubTotal คำนวณยอดรวม calcTax ภาษี calcTotal ยอดรวมทั้งหมด calcTotalWeight น้ำหนักรวม
 // addPayment ในการเพิ่มข้อมูลการชำระเงิน addOrderDetail ในการเพิ่มรายละเอียดคำสั่งซื้อ printDetail ในการแสดงรายละเอียดคำสั่งซื้อ
  ...
  //Class OrderDetail เก็บจำนวน (quantity) และสถานะภาษี (taxStatus) รวมทั้งเก็บข้อมูลสินค้าที่เป็น Item
  //calcSubTotal ในการคำนวณยอดรวม calcTax ภาษี calcTotalWeight น้ำหนักรวม 
  //addItem ในการเพิ่มข้อมูลสินค้า getDetail ในการรับข้อมูลรายละเอียดสินค้า
  ...
  //Class Item เก็บน้ำหนักส่งสินค้า (shippingWeight), รายละเอียด (description), และราคา (price) รวมทั้งเก็บข้อมูลสถานะในสต็อก (inStock)
  //setInStock ในการตั้งค่าสถานะในสต็อก getPriceForQuantity การคำนวณราคาสำหรับจำนวนที่กำหนด getTax ภาษี inStock สถานะในสต็อก
  ...
  //Class payment เก็บจำนวนเงิน (amount)
  //Class Cash การชำระด้วยเงินสดที่สืบทอดจาก Payment และมี property เพิ่มเติมคือ cashTendered
 //เมธอด printDetail ในการแสดงรายละเอียดการชำระด้วยเงินสด
 //คลาส Checkการชำระด้วยเช็คที่สืบทอดจาก Payment และมี properties เพิ่มเติมคือ name, bankID
//เมธอด authorized ในการยืนยันการชำระด้วยเช็ค
//เมธอด printDetail ในการแสดงรายละเอียดการชำระด้วยเช็ค
 //Class Credit การชำระด้วยบัตรเครดิตที่สืบทอดจาก Payment และมี properties เพิ่มเติมคือ number, type, expDate
//เมธอด authorized ในการยืนยันการชำระด้วยบัตรเครดิต
//เมธอด printDetail ในการแสดงรายละเอียดการชำระด้วยบัตรเครดิต
  ...
  // const main = () => {
  //     // สร้างข้อมูลลูกค้า
  //     let customer1 = new Customer("Ek", " NPRU");
  //     let customer2 = new Customer("Boy", "Nakhon Pathom");
  //     //console.log(customer1);
  ...
  //ฟังก์ชัน mainสร้างข้อมูลลูกค้า (Customer), สินค้า (Item), คำสั่งซื้อ (Order), และรายละเอียดคำสั่งซื้อ (OrderDetail)
//จัดการการเพิ่มข้อมูลลงในลูกค้าและคำสั่งซื้อ
//จัดการการชำระเงินด้วยเงินสดและบัตรเครดิต
//แสดงรายละเอียดคำสั่งซื้อสำหรับลูกค้า